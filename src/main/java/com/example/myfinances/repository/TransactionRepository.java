...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query("SELECT t FROM Transaction t WHERE t.account.user = :user ORDER BY t.transactionDate DESC, t.createdAt DESC")
    Page<Transaction> findTransactionsByUser(@Param("user") User user, Pageable pageable);
    
    Page<Transaction> findByAccount(Account account, Pageable pageable);
    
    @Query("SELECT t FROM Transaction t WHERE t.account.user = :user AND t.transactionDate BETWEEN :startDate AND :endDate ORDER BY t.transactionDate DESC")
    ...
    @Query("SELECT t FROM Transaction t WHERE t.account.user = :user AND t.category = :category ORDER BY t.transactionDate DESC")
    List<Transaction> findTransactionsByCategory(@Param("user") User user, @Param("category") TransactionCategory category);
    
    @Query("SELECT t FROM Transaction t WHERE t.account.user = :user AND (LOWER(t.description) LIKE LOWER(CONCAT('%', :searchTerm, '%'))) ORDER BY t.transactionDate DESC")
    ...
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.account.user = :user AND t.transactionDate BETWEEN :startDate AND :endDate")
    ...
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.account.user = :user AND t.type = :type AND t.transactionDate BETWEEN :startDate AND :endDate")
    ...
    @Query("SELECT c.name, SUM(t.amount) FROM Transaction t JOIN t.category c WHERE t.account.user = :user AND t.type = 'EXPENSE' AND YEAR(t.transactionDate) = :year GROUP BY c.name ORDER BY SUM(t.amount) DESC")
    List<Object[]> getMonthlyExpensesByCategory(@Param("user") User user, @Param("year") int year);
    
    @Query("SELECT MONTH(t.transactionDate), SUM(CASE WHEN t.type = 'INCOME' THEN t.amount ELSE 0 END), SUM(CASE WHEN t.type = 'EXPENSE' THEN t.amount ELSE 0 END) FROM Transaction t WHERE t.account.user = :user AND YEAR(t.transactionDate) = :year GROUP BY MONTH(t.transactionDate) ORDER BY MONTH(t.transactionDate)")
    List<Object[]> getMonthlyIncomeVsExpenses(@Param("user") User user, @Param("year") int year);
    
    @Query("SELECT c.name, SUM(t.amount) FROM Transaction t JOIN t.category c WHERE t.account.user = :user AND t.type = 'EXPENSE' AND t.transactionDate BETWEEN :startDate AND :endDate GROUP BY c.name ORDER BY SUM(t.amount) DESC")
